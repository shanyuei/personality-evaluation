...
import { h as getArticleBySlug, c as getLatestArticles, u as updatePreviewBySlug, d as getImageUrl, f as formatDate } from './blog-CfX7r_ne.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<main${ssrRenderAttrs(mergeProps({ class: "page-container uno-py-46px blog-page" }, _attrs))}><section><p class="uno-font-normal uno-text-[14px] md:uno-text-[16px] uno-text-[#011813] uno-leading-[20px] md:uno-leading-[24px]">${ssrInterpolate(_ctx.$t("pages.blog.detail.breadcrumb"))}</p></section><section class="uno-my-[24px] md:uno-my-[50px]">`);
      if (unref(article)) {
        _push(`<div class="uno-rounded-[24px] uno-overflow-hidden">`);
        if (unref(article)?.cover?.url) {
          _push(ssrRenderComponent(_component_NuxtImg, {
            src: ("getImageUrl" in _ctx ? _ctx.getImageUrl : unref(getImageUrl))(unref(article).cover?.url || ""),
      ...
      _push(`<div class="uno-flex uno-flex-col md:uno-flex-row uno-gap-[24px] uno-mt-56px">`);
      if (unref(article)) {
        _push(`<div class="uno-flex-1 uno-min-w-0"><p class="uno-font-normal uno-text-[14px] md:uno-text-[16px] uno-text-[#4e5255] uno-leading-[20px] md:uno-leading-[24px]">${ssrInterpolate(unref(article).updatedBy.firstname + " " + unref(article).updatedBy.lastname)} · ${ssrInterpolate(("formatDate" in _ctx ? _ctx.formatDate : unref(formatDate))(unref(article).publishedAt, "relative"))}</p><h1 class="uno-font-semibold uno-text-[28px] md:uno-text-[48px] uno-text-[#011813] uno-leading-[36px] md:uno-leading-[58px] uno-mt-8px">${ssrInterpolate(unref(article).title)}</h1><p class="uno-font-normal uno-text-[14px] md:uno-text-[16px] uno-text-[#4e5255] uno-leading-[20px] md:uno-leading-[24px] uno-mt-8px">${ssrInterpolate(unref(article).excerpt)}</p><div class="uno-mt-24px md:uno-mt-40px">${unref(article).content ?? ""}</div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="uno-w-full md:uno-w-[384px] uno-shrink-0"><div class="uno-rounded-2xl uno-mb-6"><h3 class="uno-text-[24px] uno-font-semibold uno-text-gray-900 uno-mb-4">${ssrInterpolate(_ctx.$t("pages.blog.sidebar.recent"))}</h3><ul class="uno-space-y-4"><!--[-->`);
      ssrRenderList(unref(recentArticles), (recent) => {
        ...
        _push(ssrRenderComponent(_component_AppLink, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="uno-flex uno-gap-4 uno-group"${_scopeId}>`);
...
//# sourceMappingURL=_slug_-DP6A-42S.mjs.map
