        ...
        <div class="uno-p-6 md:uno-p-8 uno-grid uno-gap-6 md:uno-grid-cols-3">
          ...
          <div class="md:uno-col-span-2 uno-space-y-4">
            ...
            <div class="uno-flex uno-items-center uno-justify-between">
              <span class="uno-flex uno-justify-center uno-items-center uno-flex-row uno-gap-2 uno-py-2 uno-px-4 uno-bg-[#F0F0F0] uno-rounded-lg uno-text-[#4E5255] uno-font-['Outfit'] uno-font-medium uno-leading-normal">{{ heroBadge }}</span>
              <span class="uno-text-[#4E5255] uno-font-['Outfit'] uno-text-right uno-font-medium uno-leading-normal">{{ $t('pages.tests.questions', { count: hero.questions }) }}</span>
            </div>
            <p class="uno-text-[#323233] uno-text-sm uno-font-['Outfit'] uno-leading-normal">{{ $t('pages.tests.hero.objective') }}</p>
            <div>
              <NuxtLink to="/test/step">
                <UButton class="uno-w-[151px] uno-h-[48px] uno-flex uno-justify-center uno-items-center uno-flex-row uno-gap-1 uno-py-1 uno-px-[20px] uno-bg-[#009D77] uno-rounded-lg uno-text-[#FFFFFF] uno-text-lg uno-font-['Outfit'] uno-font-bold uno-leading-normal">{{ $t('pages.tests.cta.startNow') }}</UButton>
              </NuxtLink>
            </div>
          </div>
        </div>
        ...
        <div class="uno-grid uno-gap-6 md:uno-grid-cols-2">
          <div v-for="card in filteredTests" :key="card.id" class="uno-bg-white uno-rounded-[24px] uno-shadow-[0px_4px_12px_rgba(0,0,0,0.08)] uno-border uno-border-[var(--ui-border)]">
            <div class="uno-p-6 uno-space-y-3">
              <div class="uno-flex uno-items-center uno-justify-between">
                <span class="uno-flex uno-justify-center uno-items-center uno-flex-row uno-gap-2 uno-py-2 uno-px-4 uno-bg-[#F0F0F0] uno-rounded-lg uno-text-[#4E5255] uno-font-['Outfit'] uno-font-medium uno-leading-normal">{{ card.badge }}</span>
                <span v-if="!card.completed" class="uno-text-[#4E5255] uno-font-['Outfit'] uno-text-right uno-font-medium uno-leading-normal">{{ $t('pages.tests.questions', { count: card.questions }) }}</span>
                <span v-else class="uno-flex uno-justify-center uno-items-center uno-flex-row uno-gap-2 uno-py-2 uno-px-4 uno-bg-[rgba(0,157,119,0.08)] uno-border uno-border-solid uno-border-[var(--ui-primary)] uno-rounded-lg uno-text-[#009D77] uno-font-['Outfit'] uno-font-medium uno-leading-normal">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    fill="none"
                    version="1.1"
                    width="13.749616384506226"
                    height="12.499651670455933"
                    viewBox="0 0 13.749616384506226 12.499651670455933"
                    aria-hidden="true"
                  >
                    <g>
                      <path
