    ...
    <div
      ...
      <section>
        <h2 class="uno-font-semibold uno-text-[24px] uno-text-[#323233] uno-leading-[36px] uno-mb-[16px]">{{
          $t('account.settings.profile.title') }}</h2>
        <div class="uno-space-y-4">
          <UFormField :ui="UFormFieldTheme" :label="$t('account.settings.profile.namePlaceholder')">
            <UInput v-model="name" :ui="UInputTheme" type="text"
              :placeholder="$t('account.settings.profile.namePlaceholder')" />
          </UFormField>
          <UFormField :ui="UFormFieldTheme" :label="$t('account.settings.profile.emailPlaceholder')">
            <UInput v-model="email" :ui="UInputTheme" type="email"
              :placeholder="$t('account.settings.profile.emailPlaceholder')" />
          </UFormField>
          <div>
            <button
              ...
              :disabled="!profileValid" @click="onSaveProfile">{{ $t('account.settings.profile.submit')
              }}</button>
          </div>
      ...
      <section class="uno-mt-34px">
        <h2 class="uno-font-semibold uno-text-[24px] uno-text-[#323233] uno-leading-[36px] uno-mb-[16px]">{{
          $t('account.settings.password.title') }}</h2>
        <div class="uno-space-y-4">
          <UFormField :ui="UFormFieldTheme">
            <UInput v-model="currentPassword" :ui="UInputTheme" type="password"
              :placeholder="$t('account.settings.password.current')" />
          </UFormField>
          <UFormField :ui="UFormFieldTheme">
            <UInput v-model="newPassword" :ui="UInputTheme" type="password"
              :placeholder="$t('account.settings.password.new')" />
          </UFormField>
          <UFormField :ui="UFormFieldTheme">
            <UInput v-model="repeatNewPassword" :ui="UInputTheme" type="password"
              :placeholder="$t('account.settings.password.repeat')" />
          </UFormField>
          <button
            class="uno-w-[217px] uno-h-[56px] uno-bg-[var(--color-green-1)] uno-rounded-[8px] uno-font-bold uno-text-[18px] uno-text-[#ffffff] uno-leading-[27px]"
            :disabled="!passwordValid" @click="onChangePassword">{{ $t('account.settings.password.submit')
            }}</button>
      ...
      <section class="uno-mt-34px">
        <h2 class="uno-font-semibold uno-text-[24px] uno-text-[#323233] uno-leading-[36px] uno-mb-[16px]">{{
          $t('account.settings.language.title') }}</h2>
        <UFormField :ui="UFormFieldTheme">
        ...
        <div class="uno-mt-4">
          <button
            ...
            @click="onLanguageSaved">{{
              $t('account.settings.language.submit') }}</button>
      ...
      <section class="uno-mt-34px">
        <h2 class="uno-font-semibold uno-text-[24px] uno-text-[#323233] uno-leading-[36px] uno-mb-[16px]">{{
          $t('account.settings.delete.title') }}</h2>
        <button
          class="uno-w-[217px] uno-h-[56px] uno-bg-[var(--color-green-1)] uno-rounded-[8px] uno-font-bold uno-text-[18px] uno-text-[#ffffff] uno-leading-[27px]"
          @click="onDeleteAccount">{{
            $t('account.settings.delete.action') }}</button>
...
const { t } = useI18n()
// definePageMeta({ title: () => t('account.settings.title') as string })
// useSeoMeta({
//   title: () => t('account.settings.title') as string,
//   description: () => t('account.settings.description') as string,
//   ogTitle: () => t('account.settings.title') as string,
//   ogDescription: () => t('account.settings.description') as string
// })
...
const onSaveProfile = async () => {
  ...
  profileMessage.value = ok ? t('account.settings.profile.success') as string : t('account.settings.profile.fail') as string
}

const onChangePassword = async () => {
  if (!newPassword.value || newPassword.value !== repeatNewPassword.value) {
    passwordSuccess.value = false
    passwordMessage.value = t('account.settings.password.mismatch') as string
    return
  }
  await new Promise(resolve => setTimeout(resolve, 600))
  passwordSuccess.value = true
  passwordMessage.value = t('account.settings.password.success') as string
...
const onLanguageSaved = () => {
  ...
  languageMessage.value = t('account.settings.language.success') as string
}

const onDeleteAccount = async () => {
  await new Promise(resolve => setTimeout(resolve, 600))
  deleteMessage.value = t('account.settings.delete.success') as string
}
</script>
